...
public class WidgetList extends WidgetPanel {
    ...
    public Widget getScrollUpButton(int color) {
        WidgetTextBox box = new WidgetTextBox("<") {
            ...
            public void draw(MatrixStack stack, Screen screen) {
                ...
                stack.rotate(new Quaternion(90.0f, 0.0f, 0.0f, 1.0f));
    ...
    public void draw(MatrixStack stack, Screen screen) {
        ...
        GuiUtils.drawGradientRect(stack.getLast().getMatrix(), 0, 0, 0, listWidth, height, borderColor, borderColor);
        GuiUtils.drawGradientRect(stack.getLast().getMatrix(), 0, 1, 1, listWidth-1, height-1, backgroundColor, backgroundColor);

        // Draw scrollbars
        if(drawScrollbar) {
            int scrollBarX = listWidth + 1;
            GuiUtils.drawGradientRect(stack.getLast().getMatrix(), 0, scrollBarX, 0, listWidth + scrollbarWidth, height, backgroundColor, backgroundColor);
            ...
            GuiUtils.drawGradientRect(stack.getLast().getMatrix(), 0, scrollBarX+1, topOffset, listWidth + scrollbarWidth -1, topOffset+paddleHeight, scrollColor, scrollColor);
        }
        ...
        if(selected >= lineOffset && selected <= lastVisibleLine) {
            ...
            GuiUtils.drawGradientRect(stack.getLast().getMatrix(), 0, 1, yOffset+1, listWidth-1, yOffset+1+selectedWidget.height-1, selectedBackgroundColor, selectedBackgroundColor);
        }
